    ...
    <div className="max-w-xl animate-fade-in">
      ...
      <div className="flex items-center gap-5 mb-8">
        <div className="w-14 h-14 rounded-full border border-white/15 bg-black flex items-center justify-center relative">
          <span className="text-lg font-semibold text-white/60">{level.toFixed(level % 1 === 0 ? 0 : 1)}</span>
        ...
        <div className="flex-1">
          ...
          <p className="text-white/20 text-xs mt-1">{level} / {target}</p>
        </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href={`/project/new?sphereId=${id}`}
          className="border border-white/10 text-white/30 px-3 py-1 rounded-full text-[10px] hover:text-white/50 hover:border-white/20 transition">
          + Проект
        </Link>
      </div>
          ...
          {projects.map((p) => (
            <Link key={p.id} href={`/project/${p.id}`}
              ...
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-white/50 group-hover:text-white/70 transition">{p.title}</span>
                <span className="text-[10px] text-white/15">{p.status === "completed" ? "✓" : `${p.progress}%`}</span>
              </div>
              {(p.point_a || p.point_b) && (
                <p className="text-[10px] text-white/15 mb-2">{p.point_a || "—"} → {p.point_b || "—"}</p>
              )}
