...
export default function NewSpherePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await supabase.from("spheres").insert({ user_id: user.id, name: name.trim(), icon: "ellipse", color: "#ffffff", target_level: targetLevel, sort_order: next });
  ...
  return (
    <div className="max-w-sm animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          <label className="block text-[9px] text-white/15 uppercase tracking-widest mb-3">Цель · {targetLevel}</label>
          <div className="flex gap-1">
            {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
              <button key={n} type="button" onClick={() => setTargetLevel(n)}
                className={`flex-1 h-2 rounded-full transition-all ${n <= targetLevel ? "bg-white/30" : "bg-white/[0.04]"}`} />
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-center py-4">
          <div className="w-14 h-14 rounded-full border border-white/20 flex items-center justify-center">
            <span className="text-sm text-white/50 font-medium">{targetLevel}</span>
          </div>
        </div>
