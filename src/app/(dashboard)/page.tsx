...
export default function MatrixPage() {
  ...
  const handleLevelChange = useCallback(async (sphereId: string, newLevel: number) => {
    setSpheres((prev) => prev.map((s) => s.id === sphereId ? { ...s, current_level: newLevel } : s));
    await supabase.from("spheres").update({ current_level: newLevel, updated_at: new Date().toISOString() }).eq("id", sphereId);
  }, [supabase]);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-5 animate-fade-in">
        ...
        <p className="text-white/40 text-sm">
          {spheres.length === 0 ? "Нет сфер" : `${spheres.length} из 3 минимальных сфер`}
        </p>
        <Link href="/sphere/new"
          className="border border-white/15 text-white/60 px-5 py-2 rounded-full text-xs hover:text-white hover:border-white/30 transition-all">
          + Добавить сферу
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex flex-col items-center h-full animate-fade-in">
      ...
      <div className="flex items-center justify-between w-full mb-2">
        ...
        <Link href="/sphere/new"
          className="border border-white/10 text-white/40 px-3 py-1.5 rounded-full text-[11px] hover:text-white/70 hover:border-white/20 transition-all">
          + Сфера
        </Link>
      </div>
        ...
        {spheres.map((s) => {
          ...
          return (
            <Link key={s.id} href={`/sphere/${s.id}`}
              className="group flex flex-col items-center gap-2 hover:opacity-100 opacity-50 transition-all duration-300">
              {/* Mini node */}
              <div className="w-10 h-10 rounded-full border border-white/20 bg-black flex items-center justify-center group-hover:border-white/50 transition-all relative">
                <span className="text-[10px] text-white/60 font-medium group-hover:text-white/90 transition">
                ...
                <svg className="absolute inset-0 -rotate-90" viewBox="0 0 40 40">
                  ...
                  <circle cx="20" cy="20" r="19" fill="none" stroke="#fff" strokeWidth="0.8" opacity="0.2"
                    strokeDasharray={`${pct * 1.194} 200`}
                    strokeLinecap="round"
                    className="transition-all duration-700"
                  />
                </svg>
              </div>
              <span className="text-[10px] text-white/30 group-hover:text-white/60 transition truncate max-w-[80px] text-center">
                {s.name}
              </span>
            </Link>
          );
