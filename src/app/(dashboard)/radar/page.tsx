...
export default function RadarPage() {
  ...
  if (spheres.length < 3) return <div className="flex flex-col items-center justify-center h-full gap-4 animate-fade-in"><div className="w-12 h-12 rounded-full border border-white/10" /><p className="text-white/25 text-xs">Нужно минимум 3 сферы</p></div>;

  const total = spheres.reduce((s, sp) => s + (Number(sp.score) || 0), 0);
  const overall = Math.round((total / (spheres.length * 10)) * 100);
          ...
          return (
            <Link key={s.id} href={`/sphere/${s.id}`}
              ...
              <div className="w-7 h-7 rounded-full border border-white/15 flex items-center justify-center shrink-0">
                <span className="text-[9px] text-white/40 font-medium">{score.toFixed(score % 1 === 0 ? 0 : 1)}</span>
              </div>
              ...
              <div className="w-16 h-px bg-white/[0.04] overflow-hidden">
                <div className="h-full bg-white/20 transition-all duration-500" style={{ width: `${(score / 10) * 100}%` }} />
              </div>
              <span className="text-[9px] text-white/15 w-6 text-right">{score}</span>
            </Link>
          );
        })}
      </div>
